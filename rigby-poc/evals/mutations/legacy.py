...
itself, and ``evals/calibrate_judge.py:224`` then scored
``parsed["accept"] and structural_valid`` -- a conjunction whose deterministic term
the corruption was built to force false, so the recorded outcome was not a function
of the grader's verdict.  See plan 06 section 6.5.  :func:`_strip_labels` removes all
three.
