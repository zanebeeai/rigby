...
CLOSING IS NOT A COMMITMENT. "close_grip" stops on its own at the object's width and at every digit that is already carrying load, so a close that is too early simply ARRIVES on the object and stops -- it cannot crush it, drive through it, or shut on nothing. There is no penalty for closing sooner and there is a large one for waiting: every run that failed did so by preparing until the clock ran out, with the object already inside the opening and the hand still being adjusted. Close when object_in_grasp_m is at or below 0.025 m, and not before. That number is not a guideline: \nit is where the object sits centred between the thumb and the fingers, so both sides arrive together. \nClosing at 0.044 -- which looks close, and reads as almost there -- put 11 N on the thumb with no \nfinger loaded at all, because the thumb caught the object and the fingers passed beside it. One \nside loaded is a push however hard it presses. If the number is above 0.025, the fix is move_to, \nnot a firmer close.
