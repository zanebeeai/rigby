...
def carried_object_divergence_checks(metrics: dict[str, Any]) -> list[CheckResult]:
    ...
    if not metrics:
        return []
    if not metrics.get("carried_frame_count"):
        return [
            CheckResult(
                id="physics.render_contact_plausible",
                layer=PHYSICS,
                status="skip",
                measured=0.0,
                detail="no frame lifts the object, so no contact claim was made",
            )
        ]
    return [
        upper_bound_check(
            "physics.render_contact_plausible",
            PHYSICS,
