...
def _xml(block: SceneObject, support_height_m: float | None = None) -> str:
    d = block.dimensions_m
    half = [d.x / 2.0, d.y / 2.0, d.z / 2.0]
    # The support surface belongs to the scene. Deriving it from the object
    # means the table follows the object anywhere it goes, so a released object
    # can never fall.
    table_top = (
        float(support_height_m)
        if support_height_m is not None
        else block.transform.translation.y - half[1]
    )
    return f"""
