...
def resolve_lock_path(project_root: Path) -> Path:
    """Locate the ``uv.lock`` governing ``project_root``.

    The three tiers are one uv workspace, so a single lock sits at the workspace
    root rather than beside each member's ``pyproject.toml``. Walking up keeps
    this correct for a workspace member and for a standalone checkout without
    either needing to know which it is.

    The walk stops at the repository boundary. An unbounded search would happily
    resolve to some unrelated ``uv.lock`` further up the filesystem and hash it
    into a reproducibility snapshot, which is a worse failure than not finding
    one: the worker refuses a missing lock loudly, but would accept a wrong lock
    silently. When nothing is found the original path is returned so callers
    still report "missing" against the place it ought to be.
    """
...
class RuntimeSettings:
    ...
    def reproducibility_snapshot(self) -> dict[str, object]:
        lock_path = resolve_lock_path(self.project_root)
        return {
            "platform": {
