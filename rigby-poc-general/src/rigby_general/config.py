...
class BaseTreeIdentity:
    """A content fingerprint of the ``rigby_v2`` source this process imported."""
...
def base_tree_fingerprint() -> BaseTreeIdentity:
    """Hash every ``.py`` file of the imported ``rigby_v2`` package.
    ...
    module_file = getattr(rigby_v2, "__file__", None)
    if module_file is None:  # pragma: no cover - namespace package
        raise RuntimeError("rigby_v2 does not expose a filesystem location")
    ...
    if count == 0:  # pragma: no cover - defensive
        raise RuntimeError(f"no rigby_v2 sources found under {root}")
