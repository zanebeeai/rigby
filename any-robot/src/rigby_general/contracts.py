...
Two deliberate departures from ``rigby_core.contracts``:
...
``RobotAssetManifestV1`` is not a ``RigAssetManifestV1``
    ...
    ``rigby_core.motion.compiler.compile_motion_program`` reads from a rig. The
...
class CompilerRigProtocol(Protocol):
    ...
    ``rigby_core.motion.compiler`` annotates its ``rig`` parameter as
